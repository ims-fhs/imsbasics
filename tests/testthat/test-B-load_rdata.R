...
test_that("load_rdata() can load \"Testroutes.Rdata\" w/o path argument", {
  ...
  file <- "Testroutes"
  filename <- paste0(file, ".RData")
  path <- paste0(dirname(dirname(getwd())), "/data/")

  myroutes <- load_rdata(filename, path)
  ...
  myroutes3 <- load_rdata(file) # Also creates a warning...
  expect_equal(nrow(myroutes3), 25244)
  expect_equal(myroutes3$time[2], 1343)
  expect_warning(load_rdata(file), )
})
