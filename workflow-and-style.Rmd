...
short etc. [see also](http://r-pkgs.had.co.nz/package.html#package-structure)
...
Short version of a good answer 
[here](http://stackoverflow.com/questions/871/why-is-git-better-than-subversion)
The key difference is that it is decentralized. Imagine you are a developer on 
...
### Work in progress: Make R executable...

See <http://www.r-bloggers.com/making-r-files-executable-under-windows/>
Choosing a new file extension name (.Rexec)

First, we have to think about a new file extension name. While double-click to 
run is a nice-to-have, the default behaviour should not be overwritten. In the 
Windows universe one cannot simply attach two different behaviours to the same 
file extension but we can register new extensions and associate custom defaults 
to those. Therefore we need another, new file extension.

To make the file extension as self-explanatory as possible, I suggest using 
.Rexec for R scripts that should be executable while leaving the default system 
behaviour for .R files as is. Associating a new file type with the .Rexec extension
...
To do so, we need access to the command line interface, e.g., via cmd. Click 
Start and type cmd into the search bar. Instead of hitting enter right away, 
right click on the 'cmd.exe' search result, choose Run as administrator from 
the context menu, and click Yes on the following pop up window. The windows 
command line should pop up thereafter.
...
### H
`git hist` needs to be set first, since it is not a standard command. Run `git config --global alias.hist "log --pretty=format:'%h %ad | %s%d [%an]' --graph --date=short"` to set up this very useful command on your local git installation.
...
###Concerning fork and branch

All branches on GitHub will be copied 
...
the manual for git-clone(1) if you wonder “what’s copied when I clone a project?”.
...
* `shell > git checkout -b devXXX` Create a new branch `devXXX`, where `XXX` stands for your name. This ensures you don't work on the master branch directly, but on a development branch of your own.
* `shell > git push -u origin devXXX` This sets the upstream for your development branch and pushes it back to origin. After this step the tag of your branch on github (network graph) should be aligned with the master tag.
...
## Case 7: You want to work on different remote branches. But you don't want to change the names of the remote branches and you don't want to introduce new remote branches.

```
> git checkout -b remotebranch remoteserver/remotebranch
```
