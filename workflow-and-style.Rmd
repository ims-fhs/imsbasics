...
For further help see [Fox](http://socserv.mcmaster.ca/jfox/Courses/R/ICPSR/R-install-instructions.html).
...
### The shell: overview over commands
...
git realises that "there is a new file you should commit"!
...
* `config` : Configure git.
    ...
    is needed. Excample for a complex command (working on windows!):
        ...
        + `git config --global alias.taghist "log --date-order --graph --tags --simplify-by-decoration --pretty=format:'%ai %h %d'"`:
...
** Not yet 100% understood. See RStudio revert button**
...
```
Create a new branch `devXXX`, where `XXX` stands for your name. This ensures you don't work on the master branch directly, but on a development branch of your own.
```
