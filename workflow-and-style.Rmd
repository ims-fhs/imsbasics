...
* `config` : Configure git.
    ...
    --graph --date=short".
    - `config - -global - - unset alias.myshortname`: Delete global shortcut. 
    Does not work, if several shortcuts with the same name exist.
    - `config - -global - - unset-all alias.myshortname`: Delete all global
    shortcuts
...
will be called `master`)
    ...
    Do **not use - -ammend option after you pushed the last commit**. In this 
