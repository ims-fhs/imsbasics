...
* `difftool` is a frontend to git diff and accepts the same options and arguments.
    ...
    or `difftool -t=kdiff3 branch1 branch2 file1.txt`: Use any of those 
    combinations to access a specific file.
...
* `checkout branch`: Switching to a specific **LOCAL** branch. Update files in the working 
tree to match the version in the index or the specified tree. If no paths are given, git 
...
After installing kdiff3, you need the following wrapper to get access to the external 
gui:
```
#!/bin/sh
...
Afterwards, adapt your git config file (For convenience, I show all of my config 
file...)
```
[filter "lfs"]
