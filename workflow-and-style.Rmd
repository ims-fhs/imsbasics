...
### Remainder for SW reviews

- Funktionsschnittstellen überprüfen:
- Macht eine Funktion nur eine klar umrissene Aufgabe?
- Manipuliert eine Funktion nur Daten, welche explizit übergeben und zurückgenommen 
werden explizit mit der Aufgabe zu tun haben? Bei begründeten Ausnahmen:
    + Ist die Ausnahme tatsächlich begründet?
    + Ist die Ausnahme klar kommentiert?
- Gibt es einen Doku-Kopf für die Funktion mit mindestens folgenden detailierten 
Angaben: Funktionsname, Ein-/ und Ausgabeparameter (Typ + Sinn) 
- Sind die Tests sinnvoll und richtig - Sind Tests allgemein in diesem Umfang sinnvoll?
- Hat sich die Geschwindigkeit geändert? Wenn ja: Kritisch? Warum?
- Wurden Tests durchgeführt
- Länge Sprint 3 Wochen?
- Ist neuer Code in master integriert worden? Falls nein: do it! (merge, SQC, SCN) + finales testthat
- Derjenige, der einlädt, bereitet vor und verschickt eine Agenda
- Nur master wird getagt.
- DESCRIPTION prüfen: imports/depends
- NAMESPACE (roxygen2) prüfen: Nichts zu tun, aber alle @export prüfen ob sinnvoll.
- Der Programmierer setzt einen ersten Satz von testthat auf. Ein externer 
überlegt sich ein unabhängiges Testkonzept: Was will ich wie und wo testen?

## Handling of code and data

For code (and only code!) use Git + Github.  
For any kind of data (raw data, simulation output, ...) use Backup to `I:\ims-fhs\DATEN`.

For windows you can adapt the following script (`.BAT`):
...
rem --- /mir	Spiegelung anlegen – Zusammenfassung von /E und /PURGE
rem --- /log:	bei jedem Syncvorgang eine neue Logdatei anlegen lassen
rem --- /np	keine Fortschrittsinformationen anzeigen
rem --- /z	Copies files in Restart mode.
rem --- /tee	Informationen sowohl auf dem Bildschirm als auch in der Logdatei ausgeben
rem --- /MT[:n]	Multithreadkopien mit n Threads erstellen (Standardwert: "8").
rem --- For further help type "robocopy/?" in cmd
rem --- Save file as .BAT
robocopy "C:\02 Projekte\03 Rettung ZCH" "I:\ims-fhs\DATEN\Rettung ZCH" /e /mir 
    ... /np /z /tee /mt /log+:my_backup_log.txt"
robocopy "C:\07 Rprogress\11 Rettung ZCH" "I:\ims-fhs\DATEN\Rettung ZCH\05 Code" 
    ... /e /b /mir /np /z /tee /mt /log+:my_backup_log.txt"
pause
```
...
Use snippets (Tools => Code => Snippets) for R and Rmd documents
```
snippet headeR
	# ------------------------
	# Author:  SQC
...
Modifications not needed here as they are tracked by Git!
...
1. Code that is stable, reviewed (4 eyes!) and generic goes into packages.  
1. Code that is fresh, unstable, unreviewed, experimental or project specific goes into the project folder.  
1. R packages include code, tests (written with `testthat`) a documentation and examples (written with `Roxygen`) and testdata to use those examples. `sim911`, `data911` and `plot911` are role models.  
1. The manual goes into git, since we want people who only install the package to be able to read the documentation.
...
Difference Windows cmd, Git bash, Shell?
E.g. if you want to escape the commit message in Git bash type <esc> :wq <enter>` 
as you are inside vim.
...
ls -d |	list directories - with ' */', e.g.  `ls -al ~/.ssh`
...
Git for Windows can be downloaded here: https://git-scm.com/download/win


* Choose Option 2, Run Git from the Windows Command Prompt. Do not select 
Option 1, Use Git Bash only, when installing or upgrading Git for the Crucible 
server – this will not work with Crucible.
* Checkout windows style, commit in Unix style line endings, then
* Use winTTY.
* Enable "file system caching" and "credential manager". This means, that you 
only have to enter the password once! Afterwards git uses git-credentials and does
"everything" for you. See also SQC's `C:\08 Vortraege\2017\git config` for further 
details.
...
If you start immediatelly to work with Github: Make sure, you have the correct 
read and write rights! See Github => Settings.
...
Close all Git-Bash processes. Then

* Download the Full installer
* Run the Git installer as done above. Ensure that you install into the same location as any 
existing Git installation. You can use `where git` to locate existing installations.
...
* `blame file`: Show what revision and author last modified each line of a file.
...
* `conig --global merge.conflictstyle diff3`: Display more details in case of conflicts.
Not yet tested (SQC).
...
above the =======). See git `conflict markers` for details
    - `merge --abort`: Abbort merge (e.g. in case of unexpected conflicts)
    - `merge --no-ff`: Do not use fast-forward. Keep to separate branches after merge.
    - `merge --no-commit`: Check files as merge is done but without commit. Combination
    `merge --abort --no-commit` possible.
...
but keep the files in the folder (`--cached`). Afterwards you need a `git add .` 
and a `git commit -a -m "remove ... from tracking"` to finish the task. Before the commit
your folder looks as if everything had changed (SCN confirm).
...
* If you don't want to increase the version run
    ...
    `Version: x.y.z-1` # or
    `Version: x.y.-1` # results in x.y.0 after commiting
...
## Git: Concerning Rebase (SLC)

One there was the case, where we wanted to ammend new changes to a earlier commit (where the 
earlier commit wasn't pushed yet). For this case the following command did work out. 
Source: [STACKOVERFLOW](https://stackoverflow.com/questions/2719579/how-to-add-a-changed-file-to-an-older-not-last-commit-in-git).

`git add .` -> add the changes   
`git commit --fixup=OLDCOMMIT` -> OLDCOMMIT is something like "d5de7c1a"  
`git rebase --interactive --autosquash OLDCOMMIT^` -> Start the rebasing
  
git rebase --interactive will bring up a text editor (which can be configured) to confirm 
(or edit) the rebase instruction sequence. There is info for rebase instruction changes in 
the file; You can just save and quit the editor with `:wq`

If there are any merge-conflicts: resolve them by hand and use: `git rebase --continue`  
At the end you should be back again on your original branch where you startet. From there 
you can push. 
...
## Case 12: You want to work on a subproject or subfolder

From [SO](http://stackoverflow.com/questions/23289006/on-windows-git-error-sparse-checkout-leaves-no-entry-on-the-working-directory).
Do the following steps (tested on Windows with Git Bash):
```
git clone -n http://github.com/ims-fhs/myfolder.git # myfolder contains subdirs Pr1 and Pr2
cd Test
...
The first step grabs the whole repo (which is still required), but does importantly 
does not perform a checkout.

If you want to merge project-a into project-b you can also do (Git >2.11)
...
If you want to include it into its own directory you can do 

```
cd path/to/project-b
git remote add project-a /C:/path/to/project-a
git fetch project-a
git merge -s ours --no-commit --allow-unrelated-histories project-a/master # or whichever branch you want to merge
git read-tree --prefix=dir-B/ -u Bproject/master # put everything in ./dir-B/
git commit -m "Merge B project as our subdirectory"
git pull -s subtree Bproject master # not necessary?
git remote remove project-a
```

The "ours" merge strategy (hear the quotes around merge?) actually does not use 
any commits from the other branch. It's sometimes mentioned as "keeping the 
other's history". But it records that history as "applied".
...
Starting with Git > 2.8 you can get the following overview
```
...
file:C:/Users/myname/.gitconfig    
    alias.hist=log --pretty=format:'%h - %an, %ad: %s' --graph --date=short
file:C:/Users/myname/.gitconfig    
    difftool.kdiff3.cmd='C:/Program Files/KDiff3/kdiff3' $LOCAL $REMOTE
...
The first part is the highest level config file which you can access via  
`git config --file c:\ProgramData\Git\config --list`. Then, we have the system-, 
global-, and local config files. **The local config is the lowest level and overwrites higher config files.**  
`user.email` for example must not be set on global level, if you work on private 
and corporate repos and use different addresses there. By not configuring either 
on the global level you are reminded on local level to set it when doing the first commit.

Mostly from git reference

core.symlinks: If false, symbolic links are checked out as small plain files that contain the link text. git-update-index[1] and git-add[1] will not change the recorded type to regular file. Useful on filesystems like FAT that do not support symbolic links. The default is true, except git-clone[1] or git-init[1] will probe and set core.symlinks false if appropriate when the repository is created.

core.autocrlf: Setting this variable to "true" is the same as setting the text attribute to "auto" on all files and core.eol to "crlf". Set to true if you want to have CRLF line endings in your working directory (Needed on windows!) and the repository has LF line endings (Needed on Unix, Linux, Mac!). This variable can be set to input, in which case no output conversion is performed.
From Github: Every time you press return on your keyboard you're actually inserting an invisible character called a line ending. Historically, different operating systems have handled line endings differently. When you view changes in a file, Git handles line endings in its own way. Since you're collaborating on projects with Git and GitHub, Git might produce unexpected results if, for example, you're working on a Windows machine, and your collaborator has made a change in OS X. Default on Windows, you simply pass true to the configuration.

core.fscache [Not git ref]: Enable file system caching. File system data will be read in bulk and cached in memory for certain operations ("core.fscache" is set to "true"). This provides a significant performance boost (experimental). 

help.format: Override the default help format used by git-help[1]. Values man, info, web and html are supported. man is the default. web and html are the same.
...
rebase.autoSquash: If set to true enable --autosquash option by default. From --no-autosquash: When the commit log message begins with "squash! ..." (or "fixup! ..."), and there is a commit whose title begins with the same ..., automatically modify the todo list of rebase -i so that the commit marked for squashing comes right after the commit to be modified, and change the action of the moved commit from pick to squash (or fixup). Ignores subsequent "fixup! " or "squash! " after the first, in case you referred to an earlier fixup/squash with git commit --fixup/--squash.
...
From stackoverflow.com: On Windows you might be better off using the manager helper (git config --global credential.helper manager). This stores your credentials in the Windows credential store which has a Control Panel interface where you can delete or edit your stored credentials. With this store, your details are secured by your Windows login and can persist over multiple sessions. The manager helper included in Git for Windows 2.x has replaced the earlier wincred helper that was added in Git for Windows 1.8.1.1. A similar helper called winstore is also available online and was used with GitExtensions as it offers a more GUI driven interface. The manager helper offers the same GUI interface as winstore. You are using wincred if credentials are needed. But if you use an SSH URL, the ssh-key is used. 

filter.lfs.xxx: LFS is an open source Git extension for versioning large files. Git Large File Storage (LFS) replaces large files
such as audio samples, videos, datasets, and graphics with text pointers inside Git, while storing the file contents on a remote 
server like GitHub.com. When using a git server that supports lfs, the lfs url defaults upon clone/adding a remote. However, 
sometimes the LFS server and git server are two separate services (such as the lfs-test-server. 
After you installed git-lfs: To see the lfs url, run git lfs env

core.repositoryFormatVersion: Internal variable identifying the repository format and layout version

core.fileMode: Tells Git if the executable bit of files in the working tree is to be honored. Some filesystems lose the executable
bit when a file that is marked as executable is checked out, or checks out an non-executable file with executable bit on. 
...
core.bare: If true this repository is assumed to be bare and has no working directory associated with it. If this is the case a number of commands that require a working directory will be disabled, such as git-add[1] or git-merge[1]. This setting is automatically guessed by git-clone[1] or git-init[1] when the repository was created. By default a repository that ends in "/.git" is assumed to be not bare (bare = false), while all other repositories are assumed to be bare (bare = true).
...
core.hideDotFiles: (Windows-only) If true, mark newly-created directories and files whose name starts with a dot as hidden. If dotGitOnly, only the .git/ directory is hidden, but no other files starting with a dot. The default mode is dotGitOnly.
